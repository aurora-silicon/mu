/** @file
  J180d MCA audio complex ACPI device.
  ...
  Tests/test_j180d_media_acpi_contract.py pins this file against that C table.
  ...
  tools/verify-j180d-av-adt.py asserts the address and the register count.
  ...
  Provenance:
    ...
    Linux arch/arm64/boot/dts/apple/t602x-j414-j416.dtsi
      (J180d GPIO overrides: SDZ pinctrl_ap 57, amp IRQ 58, jack IRQ 59)
  ...
  docs/j180d-audio-bringup.md section 8 before changing anything here.
    ...
    Scope (\_SB)
    {
        Device (MCA0)
        {
            ...
            Name (_CRS, ResourceTemplate ()
            {
                ...
                // the live ADT and tools/verify-j180d-gsiv-allocation.py
            ...
            Name (_DSD, Package ()
            {
                ...
                Package ()
                {
                    ...
                    // tools/verify-j180d-av-adt.py.  Where Apple's ADT and
                    // Asahi's hand-written Linux DT disagree, BOTH are
                    // recorded rather than one being silently preferred.
                    //
                    // Only three MCA cluster nodes are instantiated on J180d
