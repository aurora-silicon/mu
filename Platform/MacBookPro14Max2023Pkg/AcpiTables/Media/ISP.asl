/** @file
  J414c (M2 Pro / T6020) FaceTime camera ISP coprocessor ACPI device.
  ...
  Tests/test_j414c_media_acpi_contract.py machine-checks that the _CRS window
  ...
  does not silently break the camera.  tests/test_j414c_av_memory_contract.py
  pins it.
  ...
  Provenance:
    ...
    the pinned J414c live ADT capture, sha256 93d96b4a...
    docs/j414c-camera-isp-bringup.md
...
DefinitionBlock ("ISP.aml", "SSDT", 0x02, "Apple", "J414ISP", 0x00000001)
{
    Scope (\_SB)
    {
        Device (ISP0)
        {
            ...
            // they exist so that tools/verify-j414c-av-adt.py and a human
