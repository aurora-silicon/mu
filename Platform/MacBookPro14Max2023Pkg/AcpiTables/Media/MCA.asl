/** @file
  J414c MCA audio complex ACPI device.
  ...
  Tests/test_j414c_media_acpi_contract.py pins this file against that C table.
  ...
  tools/verify-j414c-av-adt.py asserts the address and the register count.
  ...
  Provenance:
    ...
    Linux arch/arm64/boot/dts/apple/t602x-j414-j416.dtsi
      (J414c GPIO overrides: SDZ pinctrl_ap 57, amp IRQ 58, jack IRQ 59)
  ...
  docs/j414c-audio-bringup.md section 8 before changing anything here.
    ...
    Scope (\_SB)
    {
        Device (MCA0)
        {
            ...
            Name (_CRS, ResourceTemplate ()
            {
                ...
                // the live ADT and tools/verify-j414c-gsiv-allocation.py
            ...
            Name (_DSD, Package ()
            {
                ...
                Package ()
                {
                    ...
                    // tools/verify-j414c-av-adt.py.  Where Apple's ADT and
                    // Asahi's hand-written Linux DT disagree, BOTH are
                    // recorded rather than one being silently preferred.
                    //
                    // Only three MCA cluster nodes are instantiated on J414c
