...
#include <Library/T6032FamilyVirtualMemoryMapDefines.h>

//
// T6032 uses 9 core MMIO range(s) + 1 PCIe range(s) + DRAM +
// framebuffer + terminator = 13 descriptors, plus headroom.
...
EFI_STATUS EFIAPI MemoryPeim(IN EFI_PHYSICAL_ADDRESS UefiMemoryBase, IN UINT64 UefiMemorySize)
{
  ...
  // separate T6032 library, so without the equivalent code the valid appended
