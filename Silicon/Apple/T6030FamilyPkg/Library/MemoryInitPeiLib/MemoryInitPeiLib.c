...
#include <Library/T6030FamilyVirtualMemoryMapDefines.h>

//
// T6030 uses 4 core MMIO range(s) + 1 PCIe range(s) + DRAM +
...
EFI_STATUS EFIAPI MemoryPeim(IN EFI_PHYSICAL_ADDRESS UefiMemoryBase, IN UINT64 UefiMemorySize)
{
  ...
  // separate T6030 library, so without the equivalent code the valid appended
