...
#include <Library/T8122FamilyVirtualMemoryMapDefines.h>

//
// T8122 uses 3 core MMIO range(s) + 1 PCIe range(s) + DRAM +
// framebuffer + terminator = 7 descriptors, plus headroom.
...
EFI_STATUS EFIAPI MemoryPeim(IN EFI_PHYSICAL_ADDRESS UefiMemoryBase, IN UINT64 UefiMemorySize)
{
  ...
  // separate T8122 library, so without the equivalent code the valid appended
