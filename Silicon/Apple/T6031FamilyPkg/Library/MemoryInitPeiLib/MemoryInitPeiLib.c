...
#include <Library/T6031FamilyVirtualMemoryMapDefines.h>

//
// T6031 uses 5 core MMIO range(s) + 1 PCIe range(s) + DRAM +
// framebuffer + terminator = 9 descriptors, plus headroom.
...
EFI_STATUS EFIAPI MemoryPeim(IN EFI_PHYSICAL_ADDRESS UefiMemoryBase, IN UINT64 UefiMemorySize)
{
  ...
  // separate T6031 library, so without the equivalent code the valid appended
