...
#include <Library/T8132FamilyVirtualMemoryMapDefines.h>

//
// T8132 uses 1 core MMIO range(s) + 0 PCIe range(s) + DRAM +
// framebuffer + terminator = 4 descriptors, plus headroom.
...
EFI_STATUS EFIAPI MemoryPeim(IN EFI_PHYSICAL_ADDRESS UefiMemoryBase, IN UINT64 UefiMemorySize)
{
  ...
  // separate T8132 library, so without the equivalent code the valid appended
