...
#include <Library/T6002FamilyVirtualMemoryMapDefines.h>

//
// T6002 uses 14 core MMIO range(s) + 1 PCIe range(s) + DRAM +
// framebuffer + terminator = 18 descriptors, plus headroom.
// Generated alongside the window list above -- keep the two in step.
//
#define MAX_VIRTUAL_MEMORY_MAP_DESCRIPTORS 22
...
EFI_STATUS EFIAPI MemoryPeim(IN EFI_PHYSICAL_ADDRESS UefiMemoryBase, IN UINT64 UefiMemorySize)
{
  ...
  // separate T6002 library, so without the equivalent code the valid appended
...
#define MAP_DEVICE_RANGE(Base, Size)                                       \
  ...
  MAP_DEVICE_RANGE (APPLE_CORE_SYSTEM_MMIO_RANGE_13_BASE,  APPLE_CORE_SYSTEM_MMIO_RANGE_13_SIZE);
  MAP_DEVICE_RANGE (APPLE_CORE_SYSTEM_MMIO_RANGE_14_BASE,  APPLE_CORE_SYSTEM_MMIO_RANGE_14_SIZE);
